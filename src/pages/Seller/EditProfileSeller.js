...
export default function EditProfileSeller() {
  ...
  useEffect(() => {
    currentUser
      ...
      .then((token) => {
        ...
        return axios.get(API_URL + "/sellers/" + currentUser.uid, config);
      })
      .then((res) => {
        if (res.data.code === 200 && res.data.data.seller) {
          const { name, phone, bio } = res.data.data.seller;
          setBio(bio);
          setName(name);
          setPhone(phone);
        }
  ...
  async function updateProfileCustomer(e) {
    ...
    return axios
      .put(API_URL + "/sellers/" + currentUser.uid, request, config)
      .then((res) => {
        setLoading(false);
        console.log(res);
      })
